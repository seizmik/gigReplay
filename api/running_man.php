...
//End function list----------------------------------------------------------------------------------
    ...
    $session_id = 50; //This should be a POST-ed object. Otherwise, we can make this one giant function and have the input to be the session_id, and return a link to the master file
    $session_name = @"Surprise!";
    $user_id = 0;
    ...
    for ($i = 0; $i < count($trim_cmd_array); $i++) {
        ...
        exec("ffmpeg -i " . $trim_cmd_array[$i]['src'] . " -vcodec libx264 -vprofile high -preset slow -b:v 5000k -maxrate 5000k -bufsize 10000k -s 640x480 -threads 0 -ss " . $trim_cmd_array[$i]['seek_to'] . " -t " . $trim_cmd_array[$i]['duration'] . " " . $temp_trim_path);
    ...
    exec("ffmpeg -i ".$combined_video_path." ".$final_video_path);
